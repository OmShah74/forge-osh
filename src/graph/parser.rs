...
pub fn parse_file(path: &str, content: &str) -> ParsedFile {
    ...
    match language {
        ...
        Language::JavaScript | Language::TypeScript => parse_js_ts(path, &lines, language),
        Language::Go => parse_go(path, &lines, language),
        Language::Java => parse_java(path, &lines, language),
        Language::C | Language::Cpp => parse_c_cpp(path, &lines, language),
...
fn try_parse_rust_def(
    ...
    // Strip async
    let rest2 = if let Some(r) = rest1.strip_prefix("async ") {
        mods.set(mflags::IS_ASYNC);
        r.trim_start()
    } else {
        rest1
    };
    // Strip unsafe
    let rest3 = if let Some(r) = rest2.strip_prefix("unsafe ") {
        mods.set(mflags::IS_UNSAFE);
        r.trim_start()
    } else {
        rest2
    };
    ...
    if let Some(r) = rest4.strip_prefix("struct ") {
        ...
        let end = if line.contains(';') {
            ...
            find_block_end(all_lines, line_num)
        };
        let doc = extract_doc(all_lines, line_num);
        return Some(ParsedDef::new(
            name,
    ...
    if let Some(r) = rest4.strip_prefix("static ") {
        let r2 = if let Some(r) = r.strip_prefix("mut ") {
            mods.set(mflags::IS_MUT);
            r.trim_start()
        } else {
            r
        };
...
fn parse_python(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    for (i, raw) in lines.iter().enumerate() {
        ...
        if let Some(cap) = def_re.captures(line) {
            ...
            defs.push(ParsedDef::new(
                name, kind, i as u32, end as u32, mods, doc, container,
            ));
            continue;
        }
...
fn parse_js_ts(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    let cls_re = re!(
        r"^(?:export\s+(?:default\s+)?)?(?:abstract\s+)?class\s+([A-Za-z_$][A-Za-z0-9_$]*)(?:\s+extends\s+([A-Za-z_$][A-Za-z0-9_$.]*))?(?:\s+implements\s+([^{]+))?"
    );
    // Interface (TS)
    let intf_re =
        re!(r"^(?:export\s+)?interface\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s+extends\s+([^{]+))?");
    // Type alias (TS)
    ...
    let mth_re = re!(
        r"^\s+(?:async\s+|static\s+|private\s+|protected\s+|public\s+|readonly\s+|override\s+|abstract\s+)*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\("
    );
    // Import
        ...
        if current_class.is_some() && raw_line.starts_with(' ') || raw_line.starts_with('\t') {
            if let Some(cap) = mth_re.captures(raw_line) {
                ...
                if !is_js_keyword(&name) {
                    ...
                    let mut mods = Modifiers::default();
                    if raw_line.contains("static ") {
                        mods.set(mflags::IS_STATIC);
                    }
                    if raw_line.contains("async ") {
                        mods.set(mflags::IS_ASYNC);
                    }
                    if raw_line.contains("override ") {
                        mods.set(mflags::IS_OVERRIDE);
                    }
                    defs.push(ParsedDef::new(
                        name,
                        kind,
...
fn parse_go(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    for (i, raw) in lines.iter().enumerate() {
        ...
        if in_import_block {
            if let Some(cap) = imp_re.captures(line) {
                imports.push(ParsedImport {
                    target: cap[1].trim_matches('"').to_string(),
                    line: i as u32,
                });
            }
            continue;
        ...
        if let Some(cap) = func_re.captures(line) {
            ...
            let kind = if raw.starts_with("func (") {
                NodeKind::Method
            } else {
                NodeKind::Function
            };
            ...
            defs.push(ParsedDef::new(
                name, kind, i as u32, end as u32, mods, doc, None,
            ));
            continue;
        }
        // type
...
fn parse_java(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    let cls_re = re!(
        r"(?:public\s+|private\s+|protected\s+)?(?:abstract\s+|final\s+|static\s+)*(?:class|record)\s+([A-Za-z_]\w*)(?:\s*<[^>]*>)?(?:\s+extends\s+([A-Za-z_][\w.]*))?(?:\s+implements\s+([^{]+))?"
    );
    let intf_re =
        re!(r"(?:public\s+)?interface\s+([A-Za-z_]\w*)(?:\s*<[^>]*>)?(?:\s+extends\s+([^{]+))?");
    let enum_re = re!(r"(?:public\s+|private\s+|protected\s+)?enum\s+([A-Za-z_]\w*)");
    let mth_re = re!(
        r"^\s+(?:public\s+|private\s+|protected\s+)?(?:static\s+|final\s+|abstract\s+|synchronized\s+|native\s+|default\s+)*(?:<[^>]*>\s+)?(?:[\w<>\[\], ]+?)\s+([a-zA-Z_]\w*)\s*\("
    );
    let pkg_re = re!(r"^\s*package\s+([\w.]+)\s*;");
    let call_re = re!(r"\b([a-zA-Z_]\w*)\s*\(");
    let const_re = re!(
        r"^\s+(?:public\s+|private\s+|protected\s+)?static\s+final\s+\S+\s+([A-Z][A-Z0-9_]+)\s*="
    );
        ...
        if current_class.is_some() {
            ...
            if let Some(cap) = mth_re.captures(raw_line) {
                ...
                if !is_java_keyword(&name) && Some(&name) != current_class.as_ref() {
                    ...
                    if raw_line.contains("abstract ") {
                        mods.set(mflags::IS_ABSTRACT);
                    }
                    if raw_line.contains("final ") {
                        mods.set(mflags::IS_FINAL);
                    }
                    if raw_line.contains("synchronized ") {
                        mods.set(mflags::IS_NATIVE);
                    }
                    if pending_override {
                        mods.set(mflags::IS_OVERRIDE);
                    }
...
fn parse_c_cpp(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    let class_re = re!(
        r"^\s*(?:template\s*<[^>]*>\s*)?class\s+([A-Za-z_]\w*)(?:\s*:\s*(?:public|protected|private)\s+([A-Za-z_][\w:]*))?"
    );
    ...
    let fn_re = re!(
        r"^(?:static\s+|inline\s+|extern\s+|virtual\s+|const\s+|unsigned\s+|signed\s+)*(?:\w[\w*&: ]*?)\s+\*?([a-zA-Z_]\w*)\s*\([^;]*$"
    );
...
fn parse_csharp(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    let cls_re = re!(
        r"(?:public\s+|private\s+|protected\s+|internal\s+)?(?:abstract\s+|sealed\s+|static\s+|partial\s+)*class\s+([A-Za-z_]\w*)(?:\s*<[^>]*>)?(?:\s*:\s*([^{]+))?"
    );
    let intf_re = re!(
        r"(?:public\s+|internal\s+)?interface\s+([A-Za-z_]\w*)(?:\s*<[^>]*>)?(?:\s*:\s*([^{]+))?"
    );
    let enum_re = re!(r"(?:public\s+|private\s+|internal\s+)?enum\s+([A-Za-z_]\w*)");
    let struct_re =
        re!(r"(?:public\s+|private\s+|internal\s+)?(?:readonly\s+)?struct\s+([A-Za-z_]\w*)");
    let mth_re = re!(
        r"^\s+(?:public\s+|private\s+|protected\s+|internal\s+)?(?:static\s+|virtual\s+|override\s+|abstract\s+|async\s+|sealed\s+)*(?:[\w<>\[\]?, ]+?)\s+([a-zA-Z_]\w*)\s*\("
    );
    let prop_re = re!(
        r"^\s+(?:public\s+|private\s+|protected\s+|internal\s+)?(?:static\s+|virtual\s+|override\s+|abstract\s+)*(?:[\w<>\[\]?, ]+?)\s+([A-Z][a-zA-Z_]\w*)\s*\{"
    );
    ...
    for (i, raw) in lines.iter().enumerate() {
        ...
        if let Some(cap) = attr_re.captures(trimmed) {
            ...
            defs.push(ParsedDef::new(
                ...
                i as u32,
                i as u32,
                Modifiers(0).with(mflags::IS_PUBLIC),
                None,
                current_class.as_ref().map(|(n, _)| n.clone()),
            ));
            continue;
...
fn parse_kotlin(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    let cls_re = re!(
        r"(?:open\s+|abstract\s+|sealed\s+|data\s+|inner\s+)*class\s+([A-Za-z_]\w*)(?:\s*<[^>]*>)?(?:\s*(?:\(.*?\))?\s*:\s*([^{]+))?"
    );
    let intf_re = re!(r"interface\s+([A-Za-z_]\w*)(?:\s*<[^>]*>)?(?:\s*:\s*([^{]+))?");
    let obj_re = re!(r"(?:companion\s+)?object\s+([A-Za-z_]\w*)");
    let fun_re = re!(
        r"^\s*(?:(?:public|private|protected|internal|override|open|abstract|suspend|inline|infix|operator|tailrec)\s+)*fun\s+(?:<[^>]*>\s+)?([a-zA-Z_]\w*)\s*\("
    );
    let enum_re = re!(r"enum\s+class\s+([A-Za-z_]\w*)");
    let const_re = re!(r"^\s*(?:const\s+)?val\s+([A-Z][A-Z0-9_]+)\s*[=:]");
    let prop_re = re!(
        r"^\s+(?:(?:public|private|protected|override|open|lateinit|lazy)\s+)*(?:val|var)\s+([a-zA-Z_]\w*)\s*[=:]"
    );
    let call_re = re!(r"\b([a-zA-Z_]\w*)\s*\(");
    ...
    for (i, raw) in lines.iter().enumerate() {
        ...
        if let Some(cap) = obj_re.captures(line) {
            ...
            defs.push(ParsedDef::new(
                ...
                Modifiers(0).with(mflags::IS_STATIC),
                doc,
                None,
            ));
            continue;
        ...
        if let Some(cap) = fun_re.captures(raw_line) {
            ...
            if !is_kotlin_keyword(&name) {
                ...
                let doc = extract_doc(lines, i);
                let mut mods = Modifiers::default();
                if raw_line.contains("suspend ") {
                    mods.set(mflags::IS_ASYNC);
                }
                if raw_line.contains("override ") {
                    mods.set(mflags::IS_OVERRIDE);
                }
                if raw_line.contains("open ") {
                    mods.set(mflags::IS_VIRTUAL);
                }
                if raw_line.contains("abstract ") {
                    mods.set(mflags::IS_ABSTRACT);
                }
                if raw_line.contains("inline ") {
                    mods.set(mflags::IS_INLINE);
                }
                if raw_line.contains("public ") || !raw_line.contains("private ") {
                    mods.set(mflags::IS_PUBLIC);
                }
                defs.push(ParsedDef::new(
                    name,
        ...
        if let Some(cap) = const_re.captures(raw_line) {
            ...
            defs.push(ParsedDef::new(
                ...
                i as u32,
                i as u32,
                Modifiers(0).with(mflags::IS_CONST),
                doc,
                current_class.clone(),
            ));
            continue;
...
fn parse_swift(path: &str, lines: &[String], language: Language) -> ParsedFile {
    let imp_re = re!(r"^\s*import\s+(\w+)");
    let cls_re = re!(
        r"(?:open\s+|public\s+|internal\s+|private\s+|fileprivate\s+)?(?:final\s+)?class\s+([A-Za-z_]\w*)(?:\s*<[^>]*>)?(?:\s*:\s*([^{]+))?"
    );
    let struct_re = re!(
        r"(?:public\s+|internal\s+|private\s+)?struct\s+([A-Za-z_]\w*)(?:\s*<[^>]*>)?(?:\s*:\s*([^{]+))?"
    );
    let proto_re = re!(r"(?:public\s+|internal\s+)?protocol\s+([A-Za-z_]\w*)(?:\s*:\s*([^{]+))?");
    let enum_re =
        re!(r"(?:public\s+|internal\s+|private\s+)?(?:indirect\s+)?enum\s+([A-Za-z_]\w*)");
    let func_re = re!(
        r"^\s*(?:(?:public|private|internal|open|fileprivate|override|static|class|mutating|@objc|@discardableResult)\s+)*func\s+([a-zA-Z_]\w*)\s*[<(]"
    );
    let init_re =
        re!(r"^\s*(?:(?:public|private|internal|required|convenience|override)\s+)*init[?(]");
    let prop_re = re!(
        r"^\s+(?:(?:public|private|internal|open|static|lazy|weak|unowned)\s+)*(?:let|var)\s+([a-zA-Z_]\w*)\s*[=:]"
    );
    let call_re = re!(r"\b([a-zA-Z_]\w*)\s*\(");
    ...
    for (i, raw) in lines.iter().enumerate() {
        ...
        if let Some(cap) = func_re.captures(raw_line) {
            ...
            if !is_swift_keyword(&name) {
                ...
                let doc = extract_doc(lines, i);
                let mut mods = Modifiers::default();
                if raw_line.contains("public ") || raw_line.contains("open ") {
                    mods.set(mflags::IS_PUBLIC);
                }
                if raw_line.contains("static ") || raw_line.contains("class ") {
                    mods.set(mflags::IS_STATIC);
                }
                if raw_line.contains("override ") {
                    mods.set(mflags::IS_OVERRIDE);
                }
                if raw_line.contains("mutating ") {
                    mods.set(mflags::IS_MUT);
                }
                defs.push(ParsedDef::new(
                    name,
...
fn parse_php(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    let cls_re = re!(
        r"(?:abstract\s+|final\s+)?class\s+([A-Za-z_]\w*)(?:\s+extends\s+([A-Za-z_][\w\\]*))?(?:\s+implements\s+([^{]+))?"
    );
    let intf_re = re!(r"interface\s+([A-Za-z_]\w*)(?:\s+extends\s+([^{]+))?");
    let trait_re = re!(r"trait\s+([A-Za-z_]\w*)");
    let fn_re = re!(
        r"^\s*(?:(?:public|private|protected|static|abstract|final)\s+)*function\s+([a-zA-Z_]\w*)\s*\("
    );
    ...
    for (i, raw) in lines.iter().enumerate() {
        ...
        if let Some(cap) = func_re.captures(line) {
            ...
            if name.contains('.') {
                ...
                let mut mods = Modifiers::default();
                if line.starts_with("local ") { /* local scope */
                } else {
                    mods.set(mflags::IS_PUBLIC);
                }
                defs.push(ParsedDef::new(
...
fn parse_scala(path: &str, lines: &[String], language: Language) -> ParsedFile {
    ...
    let cls_re = re!(
        r"(?:abstract\s+|sealed\s+|final\s+)?(?:case\s+)?class\s+([A-Za-z_]\w*)(?:\s*\[.*?\])?(?:\s*\(.*?\))?(?:\s+extends\s+(\w[\w.]*))?(?:\s+with\s+(.+?))?(?:\s*\{|$)"
    );
    let trait_re =
        re!(r"(?:sealed\s+)?trait\s+([A-Za-z_]\w*)(?:\s*\[.*?\])?(?:\s+extends\s+([^{]+))?");
    let obj_re = re!(
        r"(?:case\s+)?object\s+([A-Za-z_]\w*)(?:\s+extends\s+(\w[\w.]*))?(?:\s+with\s+(.+?))?(?:\s*\{|$)"
    );
    let def_re = re!(
        r"^\s*(?:(?:override|private|protected|final|lazy|implicit|abstract)\s+)*def\s+([a-zA-Z_]\w*)\s*[\[(]?"
    );
    let val_re = re!(
        r"^\s*(?:(?:override|private|protected|final|lazy|implicit)\s+)*(?:val|var)\s+([a-zA-Z_]\w*)\s*[=:]"
    );
    ...
    for (i, raw) in lines.iter().enumerate() {
        ...
        if let Some(cap) = trait_re.captures(line) {
            ...
            let mut def =
                ParsedDef::new(name, NodeKind::Trait, i as u32, end as u32, mods, doc, None);
            def.interfaces = interfaces;
            defs.push(def);
            continue;
        }
        ...
        if let Some(cap) = cls_re.captures(line) {
            ...
            if line.contains("sealed ") || line.contains("case ") {
                mods.set(mflags::IS_FINAL);
            }
            defs.push(ParsedDef {
                name: name.clone(),
        ...
        if let Some(cap) = obj_re.captures(line) {
            ...
            defs.push(ParsedDef {
                ...
                modifiers: Modifiers(0).with(mflags::IS_STATIC),
                doc,
                container: None,
                superclass,
                interfaces,
